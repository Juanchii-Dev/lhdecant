// Script final para agregar perfumes a la base de datos
// Este script usa fetch para comunicarse con el servidor local
...
async function addPerfumesToDatabase() {
  ...
  for (const perfume of perfumes) {
    try {
      ...
      const checkResponse = await fetch(`http://localhost:5000/api/perfumes?name=${encodeURIComponent(perfume.name)}&brand=${encodeURIComponent(perfume.brand)}`);
      
      if (checkResponse.ok) {
        const existingPerfumes = await checkResponse.json();
      ...
      if (response.ok) {
        ...
        console.log(`❌ Error: ${perfume.name} - ${perfume.brand} (${response.status})`);
        errorCount++;
      }
  ...
  console.log("5. 🚀 Hacer commit y push de los cambios");
}
